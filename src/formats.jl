...
function cuda_texture_alias_type(t::Type{T}) where T
    ...
    isbitstype(T) || @error err * " Type in not `isbitstype`."
    Te = nothing
    N = 0
    ...
    while !isempty(datatypes)
        for Ti in fieldtypes(pop!(datatypes))
            if isprimitivetype(Ti)
                ...
                typeof(Ti) == DataType || @error err * " Composed of primitive type with no alias."
                if !isprimitivetype(Ti)
                    push!(datatypes, Ti)
                    break
                end
    ...
    Ta = NTuple{N,Te}
    sizeof(T) == sizeof(Ta) || @error err * " Inferred alias type and original type have different byte sizes."
    in(N, (1, 2, 4)) || @error err * " Incompatible number of elements ($N, but only 1, 2 or 4 supported)."
    return Ta                                
end
